...
`<catalog>` is an exclusive managed-state directory containing only one `<registry>.toml` human declaration + adjacent generated `<registry>.lock` per registry and `objects/{crates,rows}/`; rows cover all locked identities, while crates contain active Git-tag archives only. Retain every approved package key even when its desired version/tag list becomes empty; deletion, source-class change, or mirror/Git mix within one registry fails closed.
