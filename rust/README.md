...
- `render`:write a new complete Pages tree. Catalog registry `main` renders at the site root; other registry aliases render below `/r/<alias>/`; output must be absent.
...
`<catalog>` is exclusive managed state:one `<registry>.toml` + generated `<registry>.lock` per registry; canonical generated `downloads.json`; exact referenced `categories/<registry>/<category>.toml`; paired `admissions/<batch>.{toml,lock}`; content-addressed `objects/{crates,rows}/`. Rows cover all locked identities; crate objects contain active Git-tag archives only. Retain every package key after its desired version/tag list becomes empty; deleting a required key, changing a name's registry/category home, or changing an existing identity's source fails closed.

Schema 4 requires catalog alias `main` at `sparse+https://rust.pkg.re/`; another canonical alias `staging` maps to `sparse+https://rust.pkg.re/r/staging/`. Registry/category/package identities are scoped by catalog registry. Dependency routing prefers a same-registry package home; absent that, exactly one external-registry home is required. The source category must explicitly permit the resolved target category. Existing registries/categories/homes are monotonic; future ones may be added.
