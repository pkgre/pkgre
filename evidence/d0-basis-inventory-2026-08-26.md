...
§11 filesystem-row audit:the compact table+shared proposal above do not constitute the required literal per-service identity table. Missing or not independently frozen per compatibility/body/rollback row:every host/container ancestor+leaf path;owner/group/mode+access/default ACL at each path;mount/subvolume+quota/reserve authority;numeric host↔container mapping proof;sole writer+all ordinary/administrative/backup readers;same-filesystem device+rename/fsync proof;exact lock-wrapper derivation;and post-sandbox writable-path proof. Both body rows are entirely ABSENT. Shared proposal values are review candidates only—not deployed facts,operator approval,or deployment-ready authority—and cannot be inherited silently by a missing row.
...
§11 network/TLS-row audit:the summary above does not provide a complete literal row for every public compatibility/body/rollback service+vhost. Missing or not independently frozen per row:exact public+internal DNS record and view;frontend/backend protocol ownership;backend socket owner/mode or network-admission authority;public/private listener classification;allowed source+destination ranges with firewall owner and ruleset reference;exact edge→backend SNI/authority/Host rewrite;raw-wire health command+expected bytes;external-denial vantage+expected result;and current certificate/key owner/reader metadata. Both body rows are entirely ABSENT;rollback and compatibility rows remain partial proposals or current observations. Summary values are not deployed dynamic-service facts,operator approval,or deployment-ready authority.
...
| D0-B06 | BLOCKED | required literal per-service filesystem+network/TLS identity rows are incomplete:per-path ancestor/leaf permissions+ACLs,public/internal DNS views,firewall owner/ruleset,backend admission ownership,edge rewrite,raw-wire health,denial-vantage,and deployment proofs are absent;current exact-looking rows are compact proposal summaries only | `fixtures/d0-v1/basis-inventory/rain-identity-design/`;`fixtures/d0-v1/basis-inventory/live-deployment-network/` |
| D0-B07 | BLOCKED | body-mode public filesystem/network/TLS rows are entirely absent;compatibility/rollback proposals cannot stand in for them or silently donate values | `fixtures/d0-v1/basis-inventory/rain-identity-design/` |
...
5. **D2 GitHub target design—no settings action:**return exact intended release workflow path/name,job permissions,check context,and proposed content SHA-256;protected environment name+human reviewers+admin-bypass policy;distinct writer identity+minimal token permissions;ruleset name/target/bypass actors/reviews/CODEOWNERS/signature/FF/force/delete/admin rules;rollback order. The workflow Git commit+blob become mandatory only after the agent creates and commits the workflow and before the later `OPERATOR-HANDOFF D2-SIGNING`;provider-assigned IDs remain returned evidence after that later operator action.
...
9. **Client coverage timing:**approve `Deno current=minimum 2.9.5` for this dated D0 with an independently newer current pin required before D6,or select another exact version;choose scoped npm production fixture deadline no later than D6;explicitly acknowledge the irreversible B18 npmjs-contact incident,return nonempty containment+remediation evidence,and approve its policy disposition without erasing or relabeling the historical fact.
...
11. **Original host-tool provenance:**provide the exact original Nix 2.34.8+Git 2.54.0 package `.drv` records from a retained binary cache/collection archive so their declared sources+hashes can be verified,or explicitly approve an evidence-policy amendment accepting the exact observed output paths plus same-output surrogate source provenance. Any waiver must bind an immutable decision ID,decision-document SHA-256,exact scope=`git-host,nix-host`,the missing original evidence,accepted substitute evidence,rationale,residual risks,approver,approval UTC,policy version,and independent acceptance. The surrogate rows are currently corroboration only;the agent may not silently treat them as original derivation proof or report a waiver as evidence satisfaction.
