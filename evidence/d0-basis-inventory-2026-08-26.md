...
Flake lock v7 pins `nixpkgs=2c423e03bbafcff28bfadc6781a4a8257f205cb5` (`narHash=sha256-dt4WdcvsA8/RCe+VZZwqU0X+XMM3wBbGCWA0/sFWzGo=`) and `rust-overlay=fd2ebb9cc4323d0c5a1336138dab5c3c5a5d8bd9` (`narHash=sha256-YT4Fs2k7bi+7YzuLt93EtIRgjpwHK5ZfsQEIh5dEQSk=`). Full source URLs/SRI hashes,derivations,outputs,wrappers,and config paths are machine-authoritative in `fixtures/d0-v1/basis-inventory/toolchain-closure/inventory.json`.
...
| npm minimum Node 24.15.0/npm 12.0.2 | `packages.x86_64-linux.js-client-node-minimum` | `/nix/store/99x4lfl8mr0d44k1w7hhkcj97jpkmgpq-pkgre-js-compat-node-npm-24.15.0-12.0.2.drv`→`/nix/store/m204igzgcqxgs4glkqjhdk8fyw8gs7id-pkgre-js-compat-node-npm-24.15.0-12.0.2` | Node archive SRI=`sha256-RyZVWB+4UVWXMMSHY+DJ07wll1xZ1RgAP8CEnT5LoPY=`;npm 12.0.2 is separately overlaid from npm tarball SRI=`sha256-XbuGxx0HoZV/LpBzQJLdali9zZ68LY1ByhxuaiHTZOE=` |
| npm current Node 26.7.0/npm 12.0.2 | `packages.x86_64-linux.js-client-node-current` | `/nix/store/k275mh0nrpwd36q1nk926bmpz6lxx8ch-pkgre-js-compat-node-npm-26.7.0-12.0.2.drv`→`/nix/store/q72ykn5nq6f88dxvika5vpzj003p2wcz-pkgre-js-compat-node-npm-26.7.0-12.0.2` | Node SRI=`sha256-mCqiTdi+TIicaoqzN93/OwiWZFsg9COTVugFUsFid+4=`;same separately overlaid npm 12.0.2 |
...
|---|---|---|---|
| BLOCKED critical | `/var/lib/keys/pkgre-js-gandiv5-token` | regular file;`root:root`;`0644`;ACL=`user::rw-,group::r--,other::r--`;41 B;shared Gandi DNS-01 PAT source for Rust+JS ACME | group/other readable;shared blast radius;treat as exposed;operator containment+rotation/revocation+scope/audit required;value was not read |
| OBSERVED | `/var/lib/acme/{rust.pkg.re,js.pkg.re,dl.rust.pkg.re}/key.pem` | `acme:nginx`;`0640`;nginx reader;ACME TLS key;227-B metadata per key | private bytes not read;current path metadata only |
| BLOCKED | production catalog SSH-Ed25519 signer+`allowedSigners`+revocations | paths/principal/fingerprint/digests/custody/readers absent | operator must freeze public trust data+private custody/rotation/recovery;fixture identity cannot be reused |
| OBSERVED+BLOCKED | Rain SSH host key | `rain.pacna.org`;SSH-Ed25519 fingerprint=`SHA256:+lFmS5DwoVcWRZduvk+R0zSnHJ++C8JRL1kopXnidiI`;10 matching scans;strict TOFU fixture | continuity only;operator out-of-band attestation absent |
...
| ABSENT | selected LAN credential | no LAN instance/config/credential exists | D13 must define per-instance reader,provider,scope,path,rotation/recovery before any LAN edit |
...
|---|---|---|---|
| D0-B01 | BLOCKED critical | Gandi PAT metadata=`0644 root:root` with group/other read;rotation/revocation/scope/audit absent | `fixtures/d0-v1/basis-inventory/live-deployment-network/`;`fixtures/d0-v1/basis-inventory/git-storage/` |
| D0-B02 | BLOCKED | Rain SSH key has TOFU continuity only;operator out-of-band fingerprint attestation absent | `fixtures/d0-v1/basis-inventory/live-deployment-network/` |
...
Secrets/files:`/var/lib/keys/pkgre-js-gandiv5-token=root:root 0644`,ACL group/other read,purpose=shared Gandi DNS-01 source;critical. Never return the token value. Certificate-key leaves=`/var/lib/acme/{rust.pkg.re,js.pkg.re,dl.rust.pkg.re}/key.pem`,`acme:nginx 0640`;private values were not read.
...
1. **Critical credential containment+rotation:**repair the declarative source and live metadata so the Gandi credential has no unauthorized group/other read and only the required system credential loader can access it;rotate/revoke the exposed PAT;inspect provider scope+audit. Return:path,owner,group,mode,ACL,size,purpose,authorized reader mechanism;old credential provider ID or safe suffix only,revocation timestamp/result;new credential provider ID or safe suffix only,creation/activation timestamp,scope/zone permissions,expiry;bounded audit-event IDs/timestamps/actors/results. Never return token bytes,hash,or private material.
2. **Rain SSH attestation:**out-of-band verify `rain.pacna.org` SSH-Ed25519 fingerprint `SHA256:+lFmS5DwoVcWRZduvk+R0zSnHJ++C8JRL1kopXnidiI`. Return:authoritative source/method,operator,UTC timestamp,algorithm,fingerprint,match result;no private host key.
