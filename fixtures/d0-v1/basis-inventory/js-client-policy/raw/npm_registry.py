...
port=int(sys.argv[1]); log=sys.argv[2]; advertised=int(sys.argv[3])
 ...
 def do_GET(self):
  ...
  if len(parts)==2:
   ...
   for v in ['1.0.0','2.0.0']:
    b=blobs[(name,v)]; vers[v]={**manifest(name,v),'dist':{'tarball':f'http://localhost:{advertised}/{name}/-/{name}-{v}.tgz','shasum':hashlib.sha1(b).hexdigest(),'integrity':'sha512-'+base64.b64encode(hashlib.sha512(b).digest()).decode()}}
