...
def tgz(name,version):
 b=json.dumps({'name':name,'version':version},separators=(',',':')).encode(); out=io.BytesIO()
 ...
 def do_GET(self):
  ...
  if len(parts)==2:
   ...
   for v in ['1.0.0','2.0.0']:
    b=blobs[(name,v)]; vers[v]={'name':name,'version':v,'dist':{'tarball':f'http://127.0.0.1:{port}/{name}/-/{name}-{v}.tgz','shasum':hashlib.sha1(b).hexdigest(),'integrity':'sha512-'+base64.b64encode(hashlib.sha512(b).digest()).decode()}}
