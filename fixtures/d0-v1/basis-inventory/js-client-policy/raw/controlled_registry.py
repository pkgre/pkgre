...
def manifest(name,version):return {'name':name,'version':version}
def tgz(name,version):
 ...
 return out.getvalue()
blobs={(n,v):tgz(n,v) for n in ('age-probe','pkgre-js') for v in ('1.0.0','2.0.0')}
class H(http.server.BaseHTTPRequestHandler):
 def do_GET(self):
  with open(log,'a') as f:f.write(json.dumps({'client':self.client_address[0],'host':self.headers.get('Host'),'method':'GET','path':self.path},sort_keys=True)+'\n')
  path=self.path.strip('/');parts=path.split('/-/')
  if len(parts)==2:
   name=parts[0];version=parts[1][len(name)+1:-4]
   if (name,version) not in blobs:self.send_error(404);return
   body=blobs[(name,version)];typ='application/octet-stream'
  elif path in ('age-probe','pkgre-js'):
   name=path;vers={}
   ...
   young=(datetime.datetime.now(datetime.timezone.utc)-datetime.timedelta(days=1)).isoformat().replace('+00:00','Z')
   body=json.dumps({'name':name,'dist-tags':{'latest':'2.0.0'},'versions':vers,'time':{'created':'2020-01-01T00:00:00.000Z','modified':young,'1.0.0':'2020-01-01T00:00:00.000Z','2.0.0':young}},separators=(',',':')).encode();typ='application/json'
