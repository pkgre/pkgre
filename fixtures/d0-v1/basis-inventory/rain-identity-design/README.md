...
| JS compatibility | `pkgre-js-serve-public.service` / `pkgre-js-serve-public` | `1977:1977` | `10.131.7.4:9011` | `127.0.0.1:9111` | `/var/lib/pkgre-js-serve-public` | 2GiB | 768MiB/1GiB | 64/2048 |
...
| JS rollback | `pkgre-js-serve-public-rollback.service` / `pkgre-js-serve-public-rollback` | `1979:1979` | `10.131.7.4:9013` | `127.0.0.1:9113` | `/var/lib/pkgre-js-serve-public-rollback` | 2GiB | 768MiB/1GiB | 64/2048 |
...
Exact D0 proposals:Rust quota=4GiB;JS quota=2GiB;Rust units `MemoryHigh=384MiB`,`MemoryMax=512MiB`;JS units `768MiB`,`1GiB`;all `TasksMax=64`,`LimitNOFILE=2048`;request concurrency=`256`;stream buffer=`65536B`;overload=`bounded 503+Retry-After`;max archive=`134217728B`;reload deadline=`120s`. Same caps apply to same-ecosystem rollback. JS cap includes the revised one-Worker `372MiB` resident envelope;D4 must measure live+candidate+old-in-flight+Worker+runtime/request buffers,FDs,reload and drain;exceeding or unproved cap blocks D7 rather than silently raising it.
