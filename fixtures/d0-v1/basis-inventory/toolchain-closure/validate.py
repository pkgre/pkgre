...
# Tool path evidence+source pins+composition semantics+explicit Deno alias.
paths=load_json("logs/toolchain-paths.json");tool_by_id={x["id"]:x for x in inv["tools"]};require(len(tool_by_id)==len(inv["tools"]),"duplicate tool IDs");path_map={"git-flake":"git","rust-toolchain":"rustToolchain","node-indexer":"nodejs24","node-minimum":"nodeMinimum","node-current":"nodeCurrent","bun-minimum":"bunMinimum","bun-current":"bunCurrent","deno-minimum":"denoMinimum","deno-current":"denoCurrent","pkgre-rust":"rustPackage","pkgre-proxy":"proxyPackage","pkgre-js":"jsPackage","dev-shell":"devShell"}
...
compat=(ROOT/"config/js-compatibility-clients.nix").read_text();fixed_sources=[]
for tid in ("node-minimum","node-current","bun-minimum","bun-current","deno-minimum","deno-current"):
 t=tool_by_id[tid];sources=t["source"] if isinstance(t["source"],list) else [t["source"]]
 for source in sources:
  require(source.get("kind")=="fixed_output_archive",f"compat source kind:{tid}");url=source["url"]
  if "nodejs.org" in url:require('https://nodejs.org/dist/v${nodeVersion}/node-v${nodeVersion}-linux-${target.nodeArch}.tar.xz' in compat and url.split('/v',1)[1].split('/',1)[0] in compat and 'nodeArch = "x64";' in compat,f"source URL template absent:{tid}")
  elif "registry.npmjs.org" in url:require('https://registry.npmjs.org/npm/-/npm-${npmVersion}.tgz' in compat and 'npmVersion = "12.0.2";' in compat,f"source URL template absent:{tid}")
  elif "oven-sh/bun" in url:require('https://github.com/oven-sh/bun/releases/download/bun-v${version}/bun-linux-${target.bunArch}.zip' in compat and url.split('bun-v',1)[1].split('/',1)[0] in compat and 'bunArch = "x64";' in compat,f"source URL template absent:{tid}")
  elif "denoland/deno" in url:require('https://github.com/denoland/deno/releases/download/v${version}/deno-${target.denoArch}-unknown-linux-gnu.zip' in compat and url.split('/v',1)[1].split('/',1)[0] in compat and 'denoArch = "x86_64";' in compat,f"source URL template absent:{tid}")
  else:raise AssertionError(f"unknown compatibility fixed source URL:{url}")
  require(source["hash_sri_sha256"] in compat,f"source hash absent from config:{tid}");fixed_sources.append((source["url"],source["hash_sri_sha256"]))
provenance=load_json("source-provenance.json");require(provenance["schema"]=="pkgre-d0-toolchain-source-provenance-v1","source provenance schema");require(provenance["collection"]=={"method":["nix derivation show <retained .drv>","test -e <declared source output>","nix hash file --type sha256 <retained flat source output>"],"networkUsed":False,"projectMutationUsed":False,"scope":"retained local Nix derivations and store objects only","secretRead":False},"source provenance collection boundary")
prov={x["id"]:x for x in provenance["tools"]};require(len(prov)==len(provenance["tools"]) and set(prov)=={"nix-host","git-host","git-flake","rust-toolchain","node-indexer","dev-shell"},"source provenance tool identities")
for tid in ("nix-host","git-host"):
 require(prov[tid]["status"]=="blocked-original-derivation-missing" and not prov[tid]["originalPackageDerivationPresentAtCollection"],f"host provenance blocker:{tid}");require(prov[tid]["surrogate"]["packageOutput"]==prov[tid]["observedOutput"],f"host surrogate output:{tid}");require(tool_by_id[tid]["source"]=={"kind":"missing_original_derivation","original_drv_present":False,"provenance":f"source-provenance.json#tools[{tid}]","status":"blocked","surrogate_only":True},f"host inventory provenance:{tid}")
git_source=prov["git-flake"]["source"];require(prov["git-flake"]["status"]=="resolved-retained-source-derivation" and git_source["hashMethod"]=="flat-archive-sha256" and git_source["hash"]=="sha256-RX/bBNyHKOAH1GiGleaRLm9oByeSDypAvxHqzBdQU1c=" and git_source["urls"]==["https://www.kernel.org/pub/software/scm/git/git-2.55.0.tar.xz"],"flake Git source provenance")
rust_source=prov["rust-toolchain"];require(rust_source["status"]=="resolved-composition" and not rust_source["singleUpstreamSourceApplicable"],"Rust composition disposition");require([x["component"] for x in rust_source["components"]]==["cargo","rustc","rust-std","rust-docs","clippy","rustfmt"],"Rust component identities");require(all(x["hashMethod"]=="flat-archive-sha256" and x["sourceOutputPresentAtCollection"] and x["localHashReverified"] for x in rust_source["components"]),"Rust component source verification")
node_source=prov["node-indexer"];require(node_source["status"]=="resolved-bundled-source" and node_source["nodeSource"]["hash"]=="sha256-9tleEKBDHuEGf8aqvp92KQi0cW3TUyTh3bSxRmt2ZZ8=" and node_source["npm"]=={"componentOutput":"/nix/store/6x3bfz00nsp3qhcgg6wzvgfv67dhgy9j-nodejs-slim-24.19.0-npm","separateSourceDerivationApplicable":False,"sourceDisposition":"bundled-in-node-source","version":"11.17.0"},"Node/npm bundled provenance")
dev_source=prov["dev-shell"];require(dev_source["status"]=="resolved-composition" and not dev_source["singleUpstreamSourceApplicable"] and [x["id"] for x in dev_source["components"]]==["rust-toolchain","curl","git-flake","gnutar","nixfmt","node-indexer"] and dev_source["compositionMachinery"]==["stdenv","bash"],"devShell composition provenance");require(provenance["disposition"]=={"blocked":["nix-host","git-host"],"blockedReason":"captured original host package derivations are absent;surrogate derivations are corroboration only","resolved":["git-flake","rust-toolchain","node-indexer","dev-shell"]},"source provenance disposition")
for tid,status in (("git-flake","resolved-retained-source-derivation"),("rust-toolchain","resolved-composition"),("node-indexer","resolved-bundled-source"),("dev-shell","resolved-composition")):require(tool_by_id[tid]["source"]["status"]==status and tool_by_id[tid]["source"]["provenance"]==f"source-provenance.json#tools[{tid}]",f"inventory source cross-reference:{tid}")
...
ok("tool_paths_sources_compositions_wrappers_and_deno_alias",{"tool_count":len(inv["tools"]),"compat_fixed_source_rows":len(fixed_sources),"unique_compat_fixed_sources":len(set(fixed_sources)),"resolved_source_rows":len(provenance["disposition"]["resolved"]),"blocked_source_rows":len(provenance["disposition"]["blocked"])})
...
report=(ROOT/"REPORT.md").read_text();require(commit in report and EXPECTED_SOURCE in report,"report basis");require("Flake-supplied Git/Rust/Node/devShell source provenance is resolved" in report and "captured original host Nix 2.34.8 and Git 2.54.0 derivations are absent" in report,"report source provenance disposition");blocker={row["id"]:row for row in inv["blockers"]}["direct-source-provenance"];require("host Nix 2.34.8 and Git 2.54.0 derivations are absent" in blocker["detail"] and "flake-supplied Git/Rust/Node/devShell provenance is resolved" in blocker["detail"],"inventory direct source blocker")
